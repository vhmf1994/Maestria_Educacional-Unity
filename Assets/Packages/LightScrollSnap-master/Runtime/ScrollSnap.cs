...
namespace LightScrollSnap
{
    ...
    public class ScrollSnap : MonoBehaviour
    {
        ...
        private void SetupClickHandlers()
        {
            ...
            {
                var item = _items[i];
                var clickHandler = item.gameObject.GetComponent<ScrollItemClickHandler>() == null ?
                                   item.gameObject.AddComponent<ScrollItemClickHandler>() :
                                   item.gameObject.GetComponent<ScrollItemClickHandler>();
                var index = i;
        ...
        {
            bool isInRange = index >= 0 && index <= _items.Count - 1;

            if (!isInRange)
                Debug.Log($"{index} is not in range");
